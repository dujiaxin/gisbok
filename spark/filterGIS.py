...
class MicrosoftAcademicGraph():
  ...
  def getDataframe(self, streamName):
    return self.sqlContext.read.format('com.databricks.spark.csv').options(header='false', delimiter='\t').schema(self.getSchema(streamName)).load(self.getFullpath(streamName))
